...
resource Ec2Instance example {
    ...
    cpuCredits                      = "standard"
    hibernation                     = false
    placementGroup                  = "example-value"
    metadataHttpTokens              = "optional"
    metadataHttpPutResponseHopLimit = 1
}
```
...
| `securityGroupIds` | `any[]` | — | — | No | List of security group IDs to attach |
...
| `rootVolumeSize` | `number` | — | — | No | Root volume size in GiB |
...
| `tags` | `object` | — | — | No | Tags to apply to the instance |
| `ebsOptimized` | `boolean` | `false` | — | No | Whether the instance is optimized for Amazon EBS I/O |
| `cpuCredits` | `string` | — | `standard`, `unlimited` | No | CPU credit option for burstable instances |
| `hibernation` | `boolean` | `false` | — | No | Whether instance hibernation is enabled |
| `placementGroup` | `string` | — | — | No | The name of the placement group for cluster networking |
| `metadataHttpTokens` | `string` | `optional` | `optional`, `required` | No | HTTP tokens requirement for instance metadata service (IMDSv2) |
| `metadataHttpPutResponseHopLimit` | `number` | `1` | — | No | HTTP PUT response hop limit for instance metadata requests (1-64) |
## Cloud Properties
